...
class MedQAInstanceDataset(MedQA):
    ...
    #old_instruction = "Below is a multi-choice question and an answer. You are a medical expert. Your job is to investigate whether the model provided a valid explanation for why three of the answer choices are incorrect and one of them is correct. Critically go through reasoning steps, consider your knowledge, and see if the final answer is correct and the incorrect answers are properly ruled out or if there are any critical mistakes."
    new_eval_instruction = "Below is a multi-choice question and an answer. You are a medical expert. Critically go through reasoning steps for why the response ruled out or accepted a certain answer choice, consider your knowledge, and then verify that the all of the rationales provided in the response are correct. If the rationales are correct, then leave everything as it is. You should not attempt to answer the question. Please focus solely on if the reasoning provided in the response is exhaustive and correct."
        
    def _get_instance_test_time_objective(self, question: str):
        evaluation_instruction = "Below is a multi-choice question and an answer. You are a medical expert. Your job is to investigate whether the model provided a valid explanation for why three of the answer choices are incorrect and one of them is correct. Critically go through reasoning steps, consider your knowledge, and see if the final answer is correct and the incorrect answers are properly ruled out or if there are any critical mistakes."
        #new_eval_instruction = "Below is a multi-choice question and an answer. You are a medical expert. Critically go through reasoning steps for why the response ruled out or accepted a certain answer choice, consider your knowledge, and then verify that the all of the rationales provided in the response are correct. If the rationales are correct, then leave everything as it is. You should not attempt to answer the question or correct the final answer. Please focus solely on if the reasoning provided in the response is exhaustive and correct."
    ...
    def __getitem__(self, index):
        ...
        question = row["question"]
        choices_dict = dict(
                A=row["choices"][0], B=row["choices"][1], C=row["choices"][2], D=row["choices"][3], Question=question
            )
