...
def load_task(task_name: str, evaluation_api: EngineLM, *args, **kwargs) -> Tuple[Dataset, Dataset, Callable]:
    ...
    if "MedQA_Prompt" in task_name:
        from textgrad.loss import MultiFieldTokenParsedEvaluation
        from .medqa_prompt import MedQA_Prompt, string_based_equality_fn
        from textgrad.autograd.string_based_ops import StringBasedFunction

        task_name = task_name[4:]
        train_set = MedQA_Prompt(task_name, split="train", *args, **kwargs)
        val_set = MedQA_Prompt(task_name, split="val", *args, **kwargs)
        test_set = MedQA_Prompt(task_name, split="test", *args, **kwargs)
        role_descriptions = [
            "Question for the task",
            "Ground truth answer",
            "Reasoning and prediction from the language model"
