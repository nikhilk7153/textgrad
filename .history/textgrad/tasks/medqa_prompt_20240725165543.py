...
class MedQA_Prompt(Dataset):
    def __init__(self, root: str=None, split: str="train", *args, **kwargs):
        ...
        if split == "train":
            self.data = make_dataset("/Users/khandekarns/Documents/MedQA_GPT35/train.jsonl")
            self.data = random.sample(self.data, 200)
        
        if split == "val":
            self.data = make_dataset("/Users/khandekarns/Documents/MedQA_GPT35/val.jsonl")
            self.data = random.sample(self.data, 100)

        if split == "test":
            self.data = make_dataset("/Users/khandekarns/Documents/MedQA_GPT35/test.jsonl")

        #data_path = os.path.join(self.root, self.task_name, f"{split}.csv")
        #self.data = pd.read_csv(data_path, index_col=0)
        self._task_description = "You will answer a medical multiple choice question where there is only one correct answer choice. The last line of your response should be of the following format: 'Answer: $LETTER' (without quotes) where LETTER is one of ABCD. Think step by step explaining why three of the answer choices and wrong and one of them is correct before providing your final answer cjoice selection."
