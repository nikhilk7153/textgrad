...
def make_dataset(file_path):
    ...
    with open(file_path) as file:
        for line in file:
            ...
            choices_dict = dict(A=data["options"]["A"], B=data["options"]["B"], C=data["options"]["C"], D=data["options"]["D"]], Question=question)
...
class BigBenchHard(Dataset):
    def __init__(self, task_name: str, root: str=None, split: str="train", *args, **kwargs):
        ...
        if split == "train":
            self.train_data = make_dataset("/Users/khandekarns/Downloads/data_clean/questions/US/4_options/phrases_no_exclude_train.jsonl")
        
        if split == "val":
            self.val_data = make_dataset("/Users/khandekarns/Downloads/data_clean/questions/US/4_options/phrases_no_exclude_dev.jsonl")

        if split == "test":
            self.test_data = make_dataset("/Users/khandekarns/Downloads/data_clean/questions/US/4_options/phrases_no_exclude_test.jsonl")

        #data_path = os.path.join(self.root, self.task_name, f"{split}.csv")
        #self.data = pd.read_csv(data_path, index_col=0)
        self._task_description = "You will answer a medical multiple choice. Think step by step. The last line of your response should be of the following format: 'Answer: $LETTER' (without quotes) where LETTER is one of ABCD. Think step by step before answering."
