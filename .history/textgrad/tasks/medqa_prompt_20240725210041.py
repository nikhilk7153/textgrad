...
def parse_choice_answer(response_text):
...
def string_based_equality_fn(prediction: tg.Variable, ground_truth_answer: tg.Variable):
    return int(parse_choice_answer(prediction.value) == str(ground_truth_answer.value))
...
def make_dataset(file_path):
    ...
    with open(file_path) as file:
        for line in file:
            ...
            choices_dict = dict(A=data["options"]["A"], 
                                B=data["options"]["B"], 
                                C=data["options"]["C"], 
                                D=data["options"]["D"], 
            ...
            dataset.append({"x": question_prompt, "y": data["answer_idx"]})

    return dataset 


class MedQA_Prompt(Dataset):
    def __init__(self, root: str=None, split: str="train", *args, **kwargs):
        ...
        if split == "train":
            self.data = make_dataset("/Users/khandekarns/Documents/data_clean/US/phrases_no_exclude_train.jsonl")
            self.data = random.sample(self.data, 200)
        
        if split == "val":
            self.data = make_dataset("/Users/khandekarns/Documents/data_clean/US/phrases_no_exclude_dev.jsonl")
            self.data = random.sample(self.data, 100)

        if split == "test":
            self.data = make_dataset("/Users/khandekarns/Documents/data_clean/US/phrases_no_exclude_test.jsonl")

        #data_path = os.path.join(self.root, self.task_name, f"{split}.csv")
        #self.data = pd.read_csv(data_path, index_col=0)
        self._task_description = "You will answer a medical multiple choice question where there is only one correct answer choice. The last line of your response should be of the following format: 'Answer: $LETTER' (without quotes) where LETTER is one of ABCD. Think step by step explaining why three of the answer choices is wrong and one of them is correct before providing your final answer choice selection."
    ...
    def _check_or_download_dataset(self):
        ...
        train_examples = [{"x": ex["input"], "y": ex["target"]} for ex in examples[:100000]]
