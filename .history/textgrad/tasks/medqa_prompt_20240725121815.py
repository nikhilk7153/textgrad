...
def make_dataset(file_path):
    ...
    with open(file_path) as file:
        for line in file:
            ...
            choices_dict = dict(
                A=data["options"]["A"], B=data["options"]["B"], C=data["options"]["C"], D=data["options"]["A"]], Question=question
            )
        question_prompt = QUERY_TEMPLATE_MULTICHOICE.format(**choices_dict)

            

            dataset.append({"question": data["question"], "choices": [data["options"]["A"], data["options"]["B"], data["options"]["C"], data["options"]["D"]], "y": data["answer_idx"]})
...
class BigBenchHard(Dataset):
    def __init__(self, task_name: str, root: str=None, split: str="train", *args, **kwargs):
        ...
        self._task_description = "You will answer a medical multiple choice. The last line of your response should be of the following format: 'Answer: $LETTER' (without quotes) where LETTER is one of ABCD. Think step by step before answering."
