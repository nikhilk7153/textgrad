...
def eval_dataset(test_set, eval_fn, model, max_samples: int=None):
    ...
    #no_response = [323, 331, 345, 835, 879, 1091, 1261]
    #bad_prompt = [347, 556, 581, 776, 835, 873]
...
optimizer = tg.TextualGradientDescent(engine=llm_api_eval, parameters=[system_prompt], gradient_memory=3)
...
for epoch in range(max_epochs):
    for steps, (batch_x, batch_y) in enumerate((pbar := tqdm(train_loader, position=0))):
        ...
        for (x, y) in zip(batch_x, batch_y):
            ...
            response = model(x)
            try:
                print("Response " + str(response.value))
                print("Ground truth " + str(y.value))
                eval_output_variable = eval_fn(inputs=dict(prediction=response, ground_truth_answer=y))
