...
task = "MedQA_Prompt"
evaluation_engine = "azure-gpt-4"
test_engine = "azure-gpt-4"
batch_size = 4
max_epochs = 3
run_validation = True 
do_not_run_larger_model = True 
...
def eval_dataset(test_set, eval_fn, model, mode, max_samples: int=None):
    ...
    no_response_test = [102, 323, 345]
    bad_prompt_test = [140, 331, 556, 581, 776, 835, 873]

    no_response_val = []
    bad_prompt_val = [91]
    
    count = 1 
    tqdm_loader = tqdm(test_set, total=len(test_set), position=0)
    for sample in tqdm_loader:

    
        if (mode == "test" and count not in no_response_test and count not in bad_prompt_test) or  (mode == "val" and count not in no_response_val and count not in bad_prompt_val):
...
def run_validation_revert(system_prompt: tg.Variable, results, model, eval_fn, val_set):
    val_performance = np.mean(eval_dataset(val_set, eval_fn, model, "val"))
...
set_seed(42)
llm_api_eval = tg.get_engine(engine_name=evaluation_engine)
llm_api_test = tg.get_engine(engine_name=test_engine)
tg.set_backward_engine(llm_api_eval, override=True)

# Load the data and the evaluation function
train_set, val_set, test_set, eval_fn = load_task(task, evaluation_api=llm_api_eval)
...
if not do_not_run_larger_model:
...
results["test_acc"].append(eval_dataset(test_set, eval_fn, model, "test"))
results["validation_acc"].append(eval_dataset(val_set, eval_fn, model, "val"))
...
for epoch in range(max_epochs):
    for steps, (batch_x, batch_y) in enumerate((pbar := tqdm(train_loader, position=0))):
        ...
        test_acc = eval_dataset(test_set, eval_fn, model, "test")
...
with open(f"./figures/results_{task}_{test_engine}.json", "w") as f:
    json.dump(results, f)